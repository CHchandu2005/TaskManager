  ...
  const NavLink = ({ el }) => {
    return (
      <Link
        ...
        className={clsx(
          "w-full lg:w-3/4 flex gap-2 px-2 py-2 rounded-tr-xl items-center text-gray-800 text-base hover:bg-[#2564ed2d] hover:text-[#16a34a]",
          path === el.link.split("/")[0] ? "bg-[#16a34a] text-neutral-100" : ""
        )}
      >
        {el.icon}
        <span>{el.label}</span>
  ...
  return (
    <div className='w-full  h-full flex flex-col gap-6 p-2 pt-4'>
      <h1 className='flex gap-1 items-center'>
        <p className='bg-[#16a34a] p-2 rounded-full'>
